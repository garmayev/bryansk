function youtube_parser(url) {
    var regExp = /^.*((rutube.ru\/)|(v\/)|(\/u\/\w\/)|(embed\/)|(watch\?))\??v?=?([^#&?]*).*/;
...
function updateYandexMapAbout() {
    $.getJSON('/local/templates/.default/components/uplab.core/template.block/map-box/networkMapData.php', {}, function (data) {
    ...
    function init() {
        ...
        $('.sub-item').each(function (index) {
            ...
            myPlacemark.events.add(['click'],  function (e) {
                ...
                $('.sub-item').eq(index).find('.sub-item__hidden').stop().css('display','block');
